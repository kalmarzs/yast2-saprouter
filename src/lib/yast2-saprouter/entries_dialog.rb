...
require "yast"
require "yast2-saprouter/query_dialog.rb"
require "yast2-saprouter/query_presenter.rb"
...
module SapRouter
  ...
  private
    ...
    def create_dialog
      Yast::UI.OpenDialog(
        ...
        VBox(
          ...
          # Footer buttons
          HBox(
            HWeight(1, PushButton(Id(:save), Yast::Label.SaveButton)),
            HStretch(),
            HWeight(1, PushButton(Id(:refresh), Yast::Label.RefreshButton)),
            HStretch(),
    ...
    def read_entries
      log.info "Calling saprouter with '#{@query.saprouter_args}'"
      @entries = @query.entries
#      @entries = File.open('/etc/saprouter/saprouttab').read
      log.info "Call to saprouter returned #{@entries.size} entries."
    rescue => e
      log.warn e.message
      @entries = []
